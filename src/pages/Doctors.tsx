...
export default function Doctors() {
    ...
    return (
        <>
            <h2>لیست دکتر ها</h2>
            <ul>
                {doctors.map(item => item.schedulesCount > 0 && <div key={item.id}><li className='li'>{item.fullName}<span> با شماره مجوز پزشکی </span>{item.medicalLicenseNumber}<span> تلفن </span>{item.phoneNumber}<span> تخصص </span>{item.specialty.name}</li></div>)}
            </ul>
